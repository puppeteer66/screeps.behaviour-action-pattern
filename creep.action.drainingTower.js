let action = new Creep.Action('drainingTower');
...
action.newTarget = function(creep){
    ...
    if ( !flag ) {
        flag = FlagDir.find(FLAG_COLOR.invade.drainingTower, creep.pos, false, FlagDir.rangeMod, {
            rangeModPerCrowd: 400
            //rangeModByType: creep.data.creepType
        });
    }

    if( creep.action && creep.action.name == 'drainingTower' && creep.flag )
        return creep.flag;
